...
describe("pricing/probability", () => {
  ...
  it("returns zero once time has expired", () => {
    const payoff = [{ underlyingPrice: 100, profitLoss: 10, legProfitLoss: [10] }];
    expect(calculateChanceOfProfit(payoff, [100], 100, 0.2, 0, 0.03)).toBe(0);
  });
});
